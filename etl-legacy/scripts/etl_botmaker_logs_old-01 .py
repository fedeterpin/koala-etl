...
def etl_messages(cur: pyodbc.Cursor) -> None:
    ...
    for m in data:
        ...
        if media:
            ...
            # Archiva el archivo de media
            path = download_and_store(url, TENANT_ID, mid, "media")
            if path:
                # MESSAGE_FILES (MERGE en PK: tenant_id, messageId, file_type)
                cur.execute("""
                   ...
                   AND target.file_type = src.file_type
                    WHEN MATCHED THEN
                      UPDATE SET
                        ...
                        downloaded_at = src.downloaded_at
                    WHEN NOT MATCHED THEN
                      INSERT (
                        tenant_id, messageId, file_type,
                        original_url, file_path, downloaded_at
                      ) VALUES (
                        src.tenant_id, src.messageId, src.file_type,
                        src.original_url, src.file_path, src.downloaded_at
                      );
                """,
                TENANT_ID, mid, url, path, datetime.now(timezone.utc)
                )
        ...
        if audio_url:
            path = download_and_store(audio_url, TENANT_ID, mid, "audio")
            if path:
                cur.execute("""
                    MERGE dbo.message_files AS target
                    USING (SELECT
                ...
                """,
                TENANT_ID, mid, audio_url, path, datetime.now(timezone.utc)
                )
...
def download_and_store(url: str, tenant_id: str, message_id: str, kind: str) -> str | None:
    """
    ...
    try:
        ...
        # Carpeta absoluta: C:\KoalaETL\files\<tenant_id>\<message_id>\<kind>
        ...
        # Retorna la ruta RELATIVA a “C:\KoalaETL”
        ...
        if resp.status_code in (403, 404):
            logger.error("Recurso inaccesible %s (%s)", url, e)
            return None
        else:
