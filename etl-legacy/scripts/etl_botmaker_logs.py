...
# Flag para controlar commits dentro de las etapas (default: False)
COMMIT_IN_STAGES = os.getenv("COMMIT_IN_STAGES", "False").lower() in ("1", "true", "yes")

# Carpeta de errores por etapa
ERR_LOG_DIR = log_dir / "errors"
...
# Estados permitidos por la CHECK en dbo.message_files.status
ALLOWED_FILE_STATUS = {"ok", "forbidden", "not_found", "error", "skipped"}

def safe_status(s: str | None) -> str:
    """Devuelve s si está permitido; si no, 'error'."""
    if not s:
        return "error"
    return s if s in ALLOWED_FILE_STATUS else "error"
...
def _get_stage_err_logger(stage: str) -> logging.Logger:
    """
    Logger exclusivo por etapa. Escribe en logs/errors/<stage>.err.log
    No propaga al logger general para evitar duplicados.
    """
    name = f"KoalaETL.{stage}.errors"
    lg = logging.getLogger(name)
    if not lg.handlers:
        fh = logging.FileHandler(ERR_LOG_DIR / f"{stage}.err.log", encoding="utf-8")
        fh.setLevel(logging.ERROR)
        fh.setFormatter(formatter)  # mismo formato de tiempo que tu logger
        lg.addHandler(fh)
        lg.setLevel(logging.ERROR)
        lg.propagate = False
    return lg

def log_stage_exception(stage: str, exc: Exception, where: str | None = None, meta: dict | None = None) -> None:
    """
    Registra la excepción en el log de la etapa como una línea con datos útiles para reproceso.
    """
    errlog = _get_stage_err_logger(stage)
    payload = {
        "where": where,
        "meta": meta,
    ...
    # Si es HTTPError, agrego status y opcionalmente el body (controlado por LOG_HTTP_BODY)
    if isinstance(exc, requests.HTTPError) and getattr(exc, "response", None) is not None:
        payload["status"] = getattr(exc.response, "status_code", None)
        if LOG_HTTP_BODY:
            payload["body"] = (exc.response.text or "")[:500]
    try:
        errlog.error("stage=%s | %s", stage, json.dumps(payload, ensure_ascii=False))
    except Exception:
        # fallback: por si algo raro en el json
        errlog.error("stage=%s | where=%s | meta=%s | exc=%s", stage, where, meta, repr(exc))
...
def set_last_ts(cur: pyodbc.Cursor, endpoint: str, ts: datetime, *, do_commit: bool = False) -> None:
    ...
    if do_commit and COMMIT_IN_STAGES:
        cur.commit()
# ——————————————————————————————————————————————————————————————
...
def api_request(method: str, url: str, **kwargs) -> requests.Response:
    ...
    r = None  # para tener referencia al último response si agotamos reintentos
    ...
    if LOG_HTTP_BODY and r is not None:
        logger.error("Body final: %s", (r.text or "")[:500])
    # Si r es None, levantamos una excepción genérica
    if r is None:
    ...
    for m in data:
        ...
        if media:
            ...
            try:
                ...
                """,
                # UPDATE
                TENANT_ID, mid, url, db_path, datetime.now(timezone.utc), dl_status_norm,
                # INSERT
                url, db_path, datetime.now(timezone.utc), dl_status_norm)
            except Exception as e:
                # No frenamos la etapa: registramos y seguimos
                logger.exception("Fallo procesando media para messageId=%s url=%s", mid, url)
                try:
                    # Si tenés log estructurado por etapa, lo aprovechamos
                    log_stage_exception(
                        "messages", e, where="message_files(media)",
                        meta={"messageId": mid, "url": url, "file_type": "media",
                              "status": dl_status if 'dl_status' in locals() else None}
                    )
                except Exception:
                    pass
        ...
        if audio_url:
            try:
                ...
                """,
                # UPDATE
                TENANT_ID, mid, audio_url, db_path, datetime.now(timezone.utc), dl_status_norm,
                # INSERT
                audio_url, db_path, datetime.now(timezone.utc), dl_status_norm)
            except Exception as e:
                            logger.exception("Fallo procesando audio para messageId=%s url=%s", mid, audio_url)
                            try:
                                log_stage_exception(
                                    "messages", e, where="message_files(audio)",
                                    meta={"messageId": mid, "url": audio_url, "file_type": "audio",
                                        "status": dl_status if 'dl_status' in locals() else None}
                                )
                            except Exception:
                                pass
...
def etl_chat_details(cur: pyodbc.Cursor) -> None:
    ...
    for cid in missing:
        try:
            ...
            # Si el endpoint devuelve 404, lo registramos como warning y seguimos con el próximo chat
            if getattr(e.response, "status_code", None) == 404:
                logger.warning("ChatId %s no encontrado (404) – se omite.", cid)
                continue
            # Otros HTTP errors se repropagan para que la etapa los trate según tu orquestador
            raise

        # Si por alguna razón vino 404 sin lanzar excepción (defensivo)
...
def _run_stage(label: str, fn, cur) -> bool:
    try:
        fn(cur)
        return True  # éxito → main() hará commit
    except requests.HTTPError as e:
        # Log estructurado por etapa
        log_stage_exception(label, e, where="HTTP")
        ...
        # Log estructurado por etapa
        log_stage_exception(label, e, where="UNCAUGHT")
